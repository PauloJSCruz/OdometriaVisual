...
pointsTestImage0 = np.array([[  [282.0   , 185.0   ],
                                [289.0   , 150.0  ],
                                [313.0   , 153.0  ],
                                [390.0  ,  226.0  ],
                                [461.0   , 242.0   ],
                                ...
                                [915.0 ,   111.0  ],
                                [994.0 ,   87.0  ],
                                [945.0  ,  296.0  ]]], dtype=np.float32)

pointsTestImage1 = np.array([[  [277.0 ,   188.0 ],
                                [285.0 ,   152.0   ],
                                [310.0  ,  155.0 ],
                                [387.0  ,  230.0 ],
                                [458.0  ,  246.0  ],
...
class VisualOdometry (Camera):
    ...
    def TrackingFutures(self):
        ...
        if( self.idFrame > 1 ):
            if ( len(self.featuresTracked[self.idFrameTracked]) < 50 ):
                self.DetectingFuturesFast()
    ...
    def MatrixEssencial(self):
        ...
        self.essencialMatrix, mask = cv2.findEssentialMat(self.featuresTracked[self.idFrame - 1], self.featuresTracked[self.idFrame], self.intrinsicParameters, method = cv2.RANSAC, prob = 0.999, threshold = 1)
...
# region
with open('matlab1.txt', 'w') as arquivo:
    pass
def matlab1 ( idFrame, vo):
    with open('matlab1.txt', 'a') as arquivo:
        # Escrevendo no arquivo
        arquivo.write(f'idFrame {idFrame}\n')
        arquivo.write(f'featuresTracked \n {vo.featuresTracked} \n')
    return 1

with open('matlab2.txt', 'w') as arquivo:
    pass
def matlab2 ( idFrame, vo):
    with open('matlab2.txt', 'a') as arquivo:
        # Escrevendo no arquivo
        arquivo.write(f'idFrame {idFrame}\n')        
        arquivo.write(f'essencialMatrix \n # {vo.essencialMatrix} # \n')
        arquivo.write(f'rotationMatrix \n | {vo.rotationMatrix} | \n')
        arquivo.write(f'translationMatrix \n / {vo.translationMatrix} / \n')
    return 1
# endregion
