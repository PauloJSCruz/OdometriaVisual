...
class VisualOdometry (Camera):
    ...
    def DetectingFeaturesFASTMethod(self):
        ...
        if (self.framesLoaded[self.idFrame] is not None):
            ...
            while(len(keypoints) < 10):
                keypoints = [ kp for kp in keypoints if kp.response > response ]
                response -= 5
                flag = True
    ...
    def TrackingFutures(self):
        ...
        LucasKanadeParams = dict(winSize=(21, 21),  # Slightly larger window to capture more context
                                 ...
                                 criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 30, 0.01))  # Stricter criteria for accuracy             
        ...
        opticalFlow, status, err = cv2.calcOpticalFlowPyrLK(self.framesLoaded[self.idFrame - 1], self.framesLoaded[self.idFrame],
                                                             self.featuresTracked[self.idFrame - 1], None, **LucasKanadeParams)
        # Certificando que status e err têm a dimensão correta
        status = status[:, 0]

        # Depuração: Verificar o número de pontos rastreados e os valores de erro
        print(f"Frame {self.idFrame}: {len(status)} pontos rastreados")
        print("Erro calculado:", err)

        # Removendo pontos que não foram correspondidos nas características rastreadas atuais
        good_new = opticalFlow[status == 1]
        good_old = self.featuresTracked[self.idFrame - 1][status == 1]

        # Filtragem por erro
        if err is not None:
            err = err[:, 0]
            print("Erro calculado:", err)  # Adiciona esta linha para depuração
            # Removendo pontos com erro zero ou muito baixo
            mask_valid_err = err < 1
            good_new = good_new[mask_valid_err]
            good_old = good_old[mask_valid_err]
            err = err[mask_valid_err]

            # Filtragem adicional de outliers
            threshold_err = np.median(err) * 2  # Exemplo de limiar baseado na mediana do erro
            mask_err = err < threshold_err
            good_new = good_new[mask_err]
            good_old = good_old[mask_err]

        self.featuresTracked[self.idFrame - 1] = good_old
        self.featuresTracked.append(good_new)
