...
class VisualOdometry (Camera):
    ...
    def DetectingFeaturesFASTMethod(self):
        ...
        if (self.framesLoaded[self.idFrame] is not None):
            ...
            while(len(keypoints) < 10):
                keypoints = [ kp for kp in keypoints if kp.response > 150 ]
                response -= 5
    ...
    def TrackingFutures(self):
        ...
        LucasKanadeParams = dict(winSize=(27, 27),  # Slightly larger window to capture more context
    ...
    def draw_correspondences(self, num_points=5):
        if self.idFrame < 1:
            return

        img1 = self.framesLoaded[self.idFrame - 1]
        img2 = self.framesLoaded[self.idFrame]

        # Cria uma imagem composta para visualização
        height1, width1 = img1.shape[:2]
        height2, width2 = img2.shape[:2]
        composite_image = np.zeros((max(height1, height2), width1 + width2, 3), dtype=np.uint8)
        composite_image[:height1, :width1] = img1
        composite_image[:height2, width1:width1 + width2] = img2

        # Seleciona um subconjunto aleatório dos pontos correspondentes
        num_points = min(num_points, len(self.featuresTracked[self.idFrame - 1]))
        indices = np.random.choice(len(self.featuresTracked[self.idFrame - 1]), num_points)
        ...
        for i in indices:
            ...
            cv2.line(composite_image, pt1, pt2, (0, 255, 0), 1)
            cv2.circle(composite_image, pt1, 5, (255, 0, 0), -1)
            cv2.circle(composite_image, pt2, 5, (0, 0, 255), -1)

        # Mostra a imagem com correspondências
        cv2.imshow('Correspondences', composite_image)
