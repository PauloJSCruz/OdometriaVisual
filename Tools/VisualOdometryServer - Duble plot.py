...
class Camera:
    ...
    def PrintFrame(self):
        ...
        # cv2.putText(currentFrame, f'FPS: {fps}', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.putText(currentFrame, f'Frame: {self.idFrame}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)

        frameRecized = self.ResizeImage(currentFrame, 960, 1080)
        cv2.imshow('Frame', frameRecized)
...
class VisualOdometry (Camera):
    ...
    def FramesOverlapping(self, newFrame,):    
        ...
        # O frame atual mantém suas cores para preservar os gráficos coloridos
        colorFrameNew = newFrame
        ...
        anaglyphFrame = cv2.merge((grayFrameOld, colorFrameNew[:,:,1], colorFrameNew[:,:,2]))
...
class Plots:
    def __init__(self, dataLogger):
        ...
        self.error.set_title('Error between ground truth and tranjectory')
...
class Trajectory (Plots):
    def __init__(self, dataLogger, voInstance):
        ...
        self.imageTrajectory = np.ones((1080, 1920, 3), dtype=np.uint8) * 255  # imagem branco        
    ...
    def PrintTrajectory(self):        
        ...
        colorTrajectoryRaspberry = (0, 128, 0)
        colorError = (10, 10, 10)
        ...
        # Add text with X, Y, and Z coordinates at the current position
        textValuesTrajectoryComputer = (f"Trajectory Computer X: {self.xValuesTrajectoryComputer[self.vo.idFrame]:.2f}, Y: {self.yValuesTrajectoryComputer[self.vo.idFrame]:.2f}, Z: {self.zValuesTrajectoryComputer[self.vo.idFrame]:.2f}")
        ...
        # Add text with X, Y, and Z coordinates at the current position
        textValuesTrajectoryRaspberry = (f"Trajectory Raspberry X: {self.xValuesTrajectoryRaspberry[self.vo.idFrame]:.2f}, Y: {self.yValuesTrajectoryRaspberry[self.vo.idFrame]:.2f}, Z: {self.zValuesTrajectoryRaspberry[self.vo.idFrame]:.2f}")
...
def main():
    ...
    totalDistanceTrajctoryComputer = 0.0
    totalDistanceTrajctoryRaspberry = 0.0
    distanceDifference = 0.0
    for i in range(1, len(trajectory.xValuesTrajectoryComputer)):
        totalDistanceTrajctoryComputer += math.sqrt( (trajectory.xValuesTrajectoryComputer[i] - trajectory.xValuesTrajectoryComputer[i - 1])**2 
        ...
        totalDistanceTrajctoryRaspberry += math.sqrt( (trajectory.xValuesTrajectoryRaspberry[i] - trajectory.xValuesTrajectoryRaspberry[i - 1])**2 
                                   + (trajectory.yValuesTrajectoryRaspberry[i] - trajectory.yValuesTrajectoryRaspberry[i - 1])**2 
                                   + (trajectory.zValuesTrajectoryRaspberry[i] - trajectory.zValuesTrajectoryRaspberry[i - 1])**2 )
        ...
        distanceDifference += (totalDistanceTrajctoryComputer - totalDistanceGroundTruth)
        distanceDifference += (totalDistanceTrajctoryRaspberry - totalDistanceGroundTruth)
        
    print(f"Distance travelled: groundTruth: {totalDistanceGroundTruth}, Trajecotry: {totalDistanceTrajctoryComputer}, Difference: {distanceDifference}")
    print(f"Erros mimimo no computador x: {min(trajectory.errorXComputer)}m, y: {min(trajectory.errorYComputer)}m, z: {min(trajectory.errorZComputer)}m")
    print(f"Erros máximos no computador x: {max(trajectory.errorXComputer)}m, y: {max(trajectory.errorYComputer)}m, z: {max(trajectory.errorZComputer)}m")

    print(f"Erros mimimo no Raspberry Pi x: {min(trajectory.errorXRaspberry)}m, y: {min(trajectory.errorYRaspberry)}m, z: {min(trajectory.errorZRaspberry)}m")
    print(f"Erros máximos  no Raspberry Pi x: {max(trajectory.errorXRaspberry)}m, y: {max(trajectory.errorYRaspberry)}m, z: {max(trajectory.errorZRaspberry)}m")
